...
public class FrameAuthor extends javax.swing.JDialog {
  ...
  private void initComponents() {
    ...
    jPanel2.add(checkSomenteAtivo, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 16, 90, 28));
    ...
    jPanel1.add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 190, 460, 50));
    ...
    jScrollPane2.setViewportView(gridAuthor);
    ...
    if (gridAuthor.getColumnModel().getColumnCount() > 0) {
      ...
      gridAuthor.getColumnModel().getColumn(0).setPreferredWidth(60);
      gridAuthor.getColumnModel().getColumn(1).setResizable(false);
      gridAuthor.getColumnModel().getColumn(1).setPreferredWidth(330);
      gridAuthor.getColumnModel().getColumn(2).setResizable(false);
      gridAuthor.getColumnModel().getColumn(2).setPreferredWidth(60);
    }

    jPanel1.add(jScrollPane2, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 460, 187));
    ...
    jPanel3.add(rbCodigo, new org.netbeans.lib.awtextra.AbsoluteConstraints(100, 10, 120, -1));

    bgOrdem.add(rbNome);
    rbNome.setFont(new java.awt.Font("Monospaced", 0, 12)); // NOI18N
    rbNome.setText("Nome");
    rbNome.setActionCommand("name");
    jPanel3.add(rbNome, new org.netbeans.lib.awtextra.AbsoluteConstraints(300, 10, 120, -1));

    jPanel1.add(jPanel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 240, 460, 40));
    ...
    jPanel4.add(txtName, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, 70, 320, 28));

    checkAtivo.setFont(new java.awt.Font("Monospaced", 0, 12)); // NOI18N
    checkAtivo.setText("Ativo");
    checkAtivo.setEnabled(false);
    jPanel4.add(checkAtivo, new org.netbeans.lib.awtextra.AbsoluteConstraints(80, 110, -1, 28));
    ...
    jPanel4.add(btNovo, new org.netbeans.lib.awtextra.AbsoluteConstraints(5, 240, 50, 28));
    ...
    jPanel4.add(btAlterar, new org.netbeans.lib.awtextra.AbsoluteConstraints(75, 240, 50, 28));
    ...
    jPanel4.add(btSalvar, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 240, 50, 28));
    ...
    jPanel4.add(btCancelar, new org.netbeans.lib.awtextra.AbsoluteConstraints(225, 240, 50, 28));
    ...
    jPanel4.add(btExcluir, new org.netbeans.lib.awtextra.AbsoluteConstraints(300, 240, 50, 28));
    ...
    jPanel4.add(btSair, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 240, 50, 28));

    jTabbedPane1.addTab("Cadastro", jPanel4);

    getContentPane().add(jTabbedPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 0, 470, 315));
  ...
  private void gridAuthorMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_gridAuthorMouseClicked
    if (evt.getClickCount() == 2) {
      preencheForm(model.getValueAt(gridAuthor.getSelectedRow(), 0).toString());
    }
  ...
  private void btSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSalvarActionPerformed
    preparaSalvar(lblCodigo.getText(), txtName.getText().toUpperCase(), checkAtivo.isSelected());
  ...
  public static void main(String args[]) {
    java.awt.EventQueue.invokeLater(new Runnable() {
      @Override
      public void run() {
        FrameAuthor dialog = new FrameAuthor(new javax.swing.JFrame(), true);
  ...
  private void preencheGrid(String name, boolean onlyActive, String ordem) {
    ...
    authors = Author.find("name like '%" + name + "%'" + active).orderBy(ordem);
    for (Author author : authors) {
      model.addRow(new Object[]{String.format("%03d", author.getId()), author.getString("name"), Funcoes.getStatus(author.getBoolean("active"))});      
    }
    gridAuthor.setModel(model);
  }

  private void preencheForm(String id) {
    Author author = Author.findById(Integer.valueOf(id));

    if (author != null) {
      lblCodigo.setText(String.format("%03d", author.getId()));
      txtName.setText(author.getString("name"));
      checkAtivo.setSelected(author.getBoolean("active"));
  ...
  private void trataComponentes(String acao) {
    switch (acao) {
      case "preencherGrid": {
        ...
        Funcoes.desabilitaEdits(txtName);
        Funcoes.desabilitaCheck(checkAtivo);
        break;
      }
      case "preparaNovo": {
        Funcoes.habilitaEdits(txtName);
        Funcoes.limpaEdits(txtName);
  ...
  private void preparaSalvar(String id, String name, boolean active) {
    Integer user = LoggedUser.getInstance().getId();

    Author author = new Author();
    try{
      author.set("id", id, "name", name, "active", active, "user_id", user);
       if(author.saveIt()){
        trataComponentes("preparaSalvar");
        lblCodigo.setText(String.format("%03d", author.getInteger("id")));
        JOptionPane.showMessageDialog(this, "Autor Gravado com Sucesso!");
      }
    }catch(ValidationException e){
      Message.validation(this, author.errors());
    }catch(Exception e){
  ...
  private void formataGrid(JTable gridAuthor) {
    Funcoes.formatFontHeader(gridAuthor, new Font("Arial", Font.PLAIN, 12));
    Funcoes.formatFontGrid(gridAuthor, new Font("Arial", Font.PLAIN, 12));
